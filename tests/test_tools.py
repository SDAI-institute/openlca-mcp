...
async def test_store_tool_scopes_disposal_to_default_connection(monkeypatch):
    seen = {}

    def fake_dispose_all(*, connection_id=None):
        seen["connection_id"] = connection_id
        return 3

    monkeypatch.setattr(handlers.store, "dispose_all", fake_dispose_all)
    body = await lifecycle.dispose_all_results()
    assert body == {"success": True, "disposed_count": 3}
    assert seen["connection_id"] == "default"
